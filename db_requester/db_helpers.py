...
class DBHelper:
    ...
    def get_movie_by_params(self, movie_data: dict[str, Any]):
        page_size = movie_data['pageSize'] * movie_data['page']
        if movie_data['createdAt'] == 'asc':
            sort = MovieDBModel.created_at.asc()
        else:
            sort = MovieDBModel.created_at.desc()
        return (self.db_session.query(MovieDBModel).filter(MovieDBModel.genre_id == movie_data['genreId'],
                                                          MovieDBModel.location == movie_data['locations'],
                                                          MovieDBModel.price >= movie_data['minPrice'],
                                                          MovieDBModel.price <= movie_data['maxPrice'],
                            MovieDBModel.published == movie_data['published']).order_by(sort).limit(page_size).all())
